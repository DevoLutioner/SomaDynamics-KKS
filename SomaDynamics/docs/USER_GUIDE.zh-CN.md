...
面板顶部的 Timeline 兼容开关为三档（1.0.3.0 起）：`关闭 / 手动 / 自动`。

- `手动`：播放 Timeline 期间所有 Chain 部位临时运行 Spring，暂停或停止后自动恢复原
  Chain（即旧版的开关行为）。
- `自动`：只有 Timeline 实际驱动（NodesConstraints 约束命中）的角色临时切换，其他
  角色保持 Chain。
- 可在 F1 插件列表给 `Timeline spring fallback hotkey` 设置自定义快捷键，播放中一键
  翻转总开关。

该选项是全局运行兼容选项，不会改写角色卡或 XML 中保存的模式；普通场景无需开启。

## 9. 保存、复位和高级参数

- 参数实时生效，并随角色卡保存。
- XML 导出会保存五个部位、模式和逐骨高级参数，适合分享完整方案。
- 配置文件区支持一键操作：`保存当前设置 Save` 直接写入预设目录；`应用所选 Apply`
  立即套用；`设为默认` 后，所有没有卡数据的角色加载时自动套用该预设——角色很多、
  共用一套身体参数时只需调整一次。
- 「默认值 Defaults」区提供五部位默认启用开关：只对新角色生效；勾选
  `应用到所有角色` 后全局覆盖（例如腹部偏瘦、不需要晃动时可全局默认关闭腹部）。
  已加载的角色需要重载后生效。
...
| PushUp / 胸托 | 是 | 1.0.3.6 起体型刷新期间暂停大腿/手臂/腹部求解，稳定后重采基准；只刷新胸部基准，不触碰其他部位 |
